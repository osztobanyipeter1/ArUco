...
class MultiArUcoSLAM:
    ...
    def __init__(self, calib_data_path, marker_size=10):  # osztály, ami egyidejűleg képes lokalizálni a kamerát és a térképezni a markereket
        ...
        self.marker_observations = defaultdict(list)
        ...
        self.detector = aruco.ArucoDetector(self.marker_dict, self.param_markers)  # tényleges detektor objektum
    ...
    def detect_and_estimate_poses(self, frame):
        ...
        marker_corners, marker_IDs, _ = self.detector.detectMarkers(
            gray_frame
        )  # ArUco markerek észlelése a képen.
        # marker_corners=észlelt marker sarokpontosk pixelkoordinátái.
        # marker_IDs=marker azonosítók tömbje

        detected_markers = {}  # ide kerülnek majd a markerek

        if marker_corners and marker_IDs is not None:
            for i, (corners, marker_id) in enumerate(
                zip(marker_corners, marker_IDs.flatten())
            ):  # sarokpontok átalakítása megfelelő formátumra
                corners = corners.reshape(-1, 2).astype(np.float32)
                ret, rvec, tvec = cv.solvePnP(  # persperctive-n-point probléma megoldása
                    self.marker_points, corners, self.cam_mat, self.dist_coef
                )
    ...
    def update_marker_map(self, detected_markers):
        ...
        for marker_id, data in detected_markers.items():
            self.marker_observations[marker_id].append((data["rvec"], data["tvec"]))
            obs_list = self.marker_observations[marker_id]
            if len(obs_list) >= 5:
                rvecs = np.array([obs[0].flatten() for obs in obs_list])
                tvecs = np.array([obs[1].flatten() for obs in obs_list])
                avg_tvec = np.mean(tvecs, axis=0)
                rotmats = [cv.Rodrigues(r)[0] for r in rvecs]
                quats = [R.from_matrix(rotmat).as_quat() for rotmat in rotmats]
                avg_quat = np.mean(quats, axis=0)
                avg_quat = avg_quat / np.linalg.norm(avg_quat)  # Quaternion normalizálása
                avg_rotmat = R.from_quat(avg_quat).as_matrix()
                avg_rvec, _ = cv.Rodrigues(avg_rotmat)

                self.marker_world_positions[marker_id] = (
                    avg_rotmat,
                    avg_tvec.reshape(3, 1),
                )
                self.marker_confidence[marker_id] = min(1.0, len(obs_list) / 10.0)
            else:
                self.marker_world_positions[marker_id] = (
                    cv.Rodrigues(data["rvec"])[0],
                    data["tvec"],
                )
        ...
        if self.reference_marker_id is None:
            ...
            R_ref = cv.Rodrigues(detected_markers[self.reference_marker_id]["rvec"])[
                0
            ]
            t_ref = detected_markers[self.reference_marker_id]["tvec"]
            self.marker_world_positions[self.reference_marker_id] = (
                np.eye(3),
                np.zeros((3, 1)),
            )
        ...
        if self.reference_marker_id in detected_markers:
            ...
            for marker_id, data in detected_markers.items():
                if marker_id not in self.marker_world_positions:
                    ...
                    # Koordináta transzformációk láncolása:
                    # T_world_to_marker = T_cam_to_marker * T_world_to_cam
                ...
                if marker_id in self.marker_confidence:
                    self.marker_confidence[marker_id] = min(
                        1.0, self.marker_confidence[marker_id] + 0.1
                    )
        ...
        for marker_id in detected_markers:
            if marker_id in self.marker_world_positions:
                ...
                R_world_to_marker, t_world_to_marker = self.marker_world_positions[
                    marker_id
                ]
                ...
                t_world_to_cam = (
                    R_world_to_marker.T @ (-t_world_to_marker)
                    - R_cam_to_marker.T @ t_cam_to_marker
                )
    ...
    def update_visualization(self, camera_position=None, detected_markers=None):
        ...
        for i, (marker_id, (R, t)) in enumerate(
            self.marker_world_positions.items()
        ):  # sarokpontok
            ...
            )  # azonosító és bizalmi érték megjelenítése

            # Marker kontúr
            corners_plot = np.vstack([world_corners, world_corners[0]])
            self.ax.plot(corners_plot[:, 0], corners_plot[:, 1], corners_plot[:, 2], c=colors[i], linewidth=2)

            # Marker normálvektor megjelenítése!
            normal = R @ np.array([0, 0, 1])
            center = t.flatten()
            self.ax.quiver(
                center[0], center[1], center[2], normal[0], normal[1], normal[2], length=5, color=colors[i], alpha=0.7
            )
        ...
        if self.camera_positions:
            ...
            if len(self.camera_positions) > 1:
                self.ax.plot(
                    cam_pos_array[:, 0], cam_pos_array[:, 1], cam_pos_array[:, 2], "r-", alpha=0.5, linewidth=2, label="Kamera trajektória"
                )

            # Aktuális kamera pozíció
            self.ax.scatter(cam_pos_array[-1, 0], cam_pos_array[-1, 1], cam_pos_array[-1, 2], c="red", s=150, label="Kamera")
        ...
        if self.reference_marker_id is not None:
            self.ax.text2D(0.05, 0.95, f"Referencia marker: {self.reference_marker_id}", transform=self.ax.transAxes)
    ...
    def save_map(self, filename="aruco_map.json"):
        ...
        map_data = {
            # (referencia marker id, rotációs mátrix, transzlációs vektor, bizalmi érték, és marker méret)
    ...
    try:
        while True:
            ...
            for marker_id, data in detected_markers.items():
                ...
                # Tengelyek rajzolása
                cv.drawFrameAxes(frame, slam.cam_mat, slam.dist_coef, data["rvec"], data["tvec"], 4, 4)
                ...
                cv.putText(frame, text, tuple(corners[0]), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            ...
            for i, text in enumerate(status_text):
                cv.putText(frame, text, (10, 30 + i * 25), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
